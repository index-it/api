...
class CategoryCMImpl(
    ...
    override fun getAll(userId: IxId<UserDto>, listId: IxId<ListDto>): List<CategoryDto> = getAll(keyValue(userId, listId))

    override fun get(userId: IxId<UserDto>, listId: IxId<ListDto>, categoryId: IxId<CategoryDto>): CategoryDto? = get(keyValue(userId, listId), categoryId.toString())

    override fun cacheAll(userId: IxId<UserDto>, listId: IxId<ListDto>, categoriesDto: List<CategoryDto>) {
        cacheAll(keyValue(userId, listId), categoriesDto.associateBy { it.id.toString() })
    }

    override fun cache(userId: IxId<UserDto>, listId: IxId<ListDto>, categoryDto: CategoryDto) {
        cache(keyValue(userId, listId), categoryDto.id.toString(), categoryDto)
    }

    override fun delete(userId: IxId<UserDto>, listId: IxId<ListDto>, categoryId: IxId<CategoryDto>) {
        delete(keyValue(userId, listId), categoryId.toString())
    }

    override fun deleteMultiple(userId: IxId<UserDto>, listId: IxId<ListDto>, categoryIds: List<IxId<CategoryDto>>) {
