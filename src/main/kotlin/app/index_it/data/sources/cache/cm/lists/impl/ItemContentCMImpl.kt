...
class ItemContentCMImpl(
    ...
    override fun get(userId: IxId<UserDto>, itemId: IxId<ItemDto>): ItemContentDto? =
        get(userId.toString(), itemId.toString())

    override fun cache(userId: IxId<UserDto>, itemContentDto: ItemContentDto) {
        cache(userId.toString(), itemContentDto.id.toString(), itemContentDto)
    }

    override fun delete(userId: IxId<UserDto>, itemId: IxId<ItemDto>) {
        delete(userId.toString(), itemId.toString())
    }

    override fun deleteMultiple(userId: IxId<UserDto>, itemIds: List<IxId<ItemDto>>) {
