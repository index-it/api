...
class ListsRoute {
    @Resource("{listId}")
    class ListRoute(val parent: ListsRoute = ListsRoute(), @Contextual val listId: IxId<ListDto>) {
        @Resource("categories")
        class CategoriesRoute(val parent: ListRoute) {
            @Resource("{categoryId}")
            class CategoryRoute(val parent: CategoriesRoute, @Contextual val categoryId: IxId<CategoryDto>)
        }

        @Resource("items")
        class ItemsRoute(val parent: ListRoute, val completed: Boolean? = null) {
            @Resource("{itemId}")
            class ItemRoute(val parent: ItemsRoute, @Contextual val itemId: IxId<ItemDto>) {
                @Resource("content")
