...
object CategoryDBM {
    ...
    fun getAll(userId: Id<UserDto>, listId: Id<ListDto>): MutableList<CategoryDto>? {
        return col.findOne(and(ListDto::id eq listId, ListDto::user_id eq userId))?.categories
    }

    fun get(userId: Id<UserDto>, listId: Id<ListDto>, categoryId: Id<CategoryDto>): CategoryDto? {
        return col.findOne(and(ListDto::id eq listId, ListDto::user_id eq userId))?.categories?.firstOrNull { it.id == categoryId }
    }
